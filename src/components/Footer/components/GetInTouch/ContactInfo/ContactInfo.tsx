...
const ContactInfo: FunctionComponent = () => {
  return contactsData.map(({ id, icon, description }) => (
    <div key={id} className={classes.ContactInfo}>
      <span className={classes.ContactInfo__icon}>{icon}</span>
      {id === "email" ? (
        <a href={`mailto:${description}`} className={classes.ContactInfo__text}>
          {description}
        </a>
      ) : id === "phone" ? (
        <a href={`tel:${description}`} className={classes.ContactInfo__text}>
          {description}
        </a>
      ) : (
        <a
